...
const MovingImg = ({ title, img, link }) => {
  ...
  return (
    ...
    >
      ...
      ref={imgRef} src={img} alt={title} className="w-96 h-auto hidden absolute rounded-lg z-10 md:!hidden sm:!hidden xs:!hidden"/>
    </Link>
  );
};
...
const Article = ({ img, title, date, link }) => {
  return (
    ...
    rounded-xl flex items-center justify-between
     bg-light dark:bg-primaryDark text-dark first:mt-0 border border-solid
      border-dark shadow-xl border-r-4 border-b-4 dark:border-light
      sm:flex-col
      ">
...
const FeaturedArticles = ({ img, title, time, summary, link }) => {
  return (
    <li className="col-span-1 w-full p-4 bg-light dark:bg-primaryDark rounded-2xl shadow-2xl relative">
...
const articles = () => {
  return (
    <>
      <Head>
        <title>DreyKin | Articles Page</title>
        <meta name="description" content="description" />
      </Head>
      <TransitionEffect/>
      <main className="w-full mb-16 flex flex-col items-center justify-center overflow-hidden">
        <Layout className="pt-16">
          <AnimatedText text="Words Can Change The World!" className="mb-16 lg:text-7xl sm:mb-8 sm:text-6xl xs:text-4xl dark:text-light" />
